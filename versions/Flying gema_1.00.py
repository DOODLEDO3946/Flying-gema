...
def pprint(place, text, size, colour, point, returnrect=False):		# easier text on screen
	...
	if place == "midtop":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midtop=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midtop=point))
	elif place == "center":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(center=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(center=point))
	elif place == "topleft":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(topleft=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(topleft=point))
	elif place == "topright":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(topright=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(topright=point))
	elif place == "midright":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midright=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midright=point))
	elif place == "midleft":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midleft=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midleft=point))
	else:#elif place == "midbottom":
		screen.blit(pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour), pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midbottom=point))
		if returnrect:
			return (pygame.font.SysFont('dejavusans.ttf', size).render(text, True, colour).get_rect(midbottom=point))
...
def settings():
	...
	while True:
		...
		if response["version"] > version:
			version_rect = pprint("midtop", "Update Available", 22, whiteColour, (320, 180), True)
		...
		for event in pygame.event.get():
			if event.type == KEYDOWN:
				if (event.key == K_UP or event.key == ord('w')) and pointer > 40:
					...
					arrowkey("left")
				elif (event.key == K_DOWN or event.key == ord('s')) and pointer < 180:
					pointer += 20
		else:
			continue
		break
